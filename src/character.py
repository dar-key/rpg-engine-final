...
class Character:
    ...
    def speed(self) -> float:
        ...
        formula = self.SPEED_FORMULAS.get(
            self.speed_formula, self.SPEED_FORMULAS["default"]
        )
    ...
    def damage(self) -> float:
        weapon_bonus = (
            self.equipped["weapon"].attack_effect if self.equipped["weapon"] else 0
        )
    ...
    def show_stats(self):
        ...
        print(
            f"  Weapon      : {self.equipped['weapon'].name if self.equipped['weapon'] else 'None'}"
        )
        print(
            f"  Armor       : {self.equipped['armor'].name  if self.equipped['armor']  else 'None'}"
        )
        print(sep)
    ...
    def show_inventory(self):
        ...
        for i, item in enumerate(self.inventory, 1):
            ...
            print(
                f"  {i}. {item.name}{equipped_tag}  ({item.item_type}, {item.price}g)"
            )
    ...
    def gain_experience(self, amount: int):
        ...
        print(
            f"  {self.name} gained {amount} XP. ({self.experience}/{self.level * 100})"
        )
    ...
    def set_speed_formula(self, formula_name: str) -> bool:
        if formula_name in self.SPEED_FORMULAS:
            ...
            return True
        print(
            f"  Unknown formula '{formula_name}'. Options: {list(self.SPEED_FORMULAS.keys())}"
        )
        return False
